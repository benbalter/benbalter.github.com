...
```json
{
  ...
  "@type": "Person",
  "name": "Ben Balter",
  "jobTitle": "Director of Hubber Enablement at GitHub",
  "worksFor": {
    "@type": "Organization",
    "name": "GitHub"
  },
...
The homepage uses a ProfilePage schema (a Google-supported rich result type) wrapping the Person entity:
...
* `src/layouts/BaseLayout.astro`: Base layout with meta tags (uses `astro-seo` package)
...
SEO meta tags are handled by the `astro-seo` package in `src/layouts/BaseLayout.astro`. The `<SEO>` component handles:

* Title, description, and canonical URL
* Open Graph tags (including `og:image` with width, height, and alt)
* Twitter Card tags (including `twitter:image:alt`)
* Additional meta tags (viewport, author, keywords, theme-color)

Structured data (JSON-LD) is generated by utilities in `src/utils/structured-data.ts`:

* `generatePersonSchema()` — Person schema for non-homepage pages
* `generateProfilePageSchema()` — ProfilePage schema for the homepage (Google rich result type)
* `generateWebSiteSchema()` — WebSite schema for all pages
* `generateBlogPostingSchema()` — BlogPosting schema for blog posts
* `generateBreadcrumbSchema()` — BreadcrumbList for navigation
* `generateResumeSchema()` — Enhanced Person schema with resume data
