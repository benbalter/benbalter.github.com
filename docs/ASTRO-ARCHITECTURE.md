...
**Implementation:** Generate redirect HTML files at build time
...
// Read all posts with redirect_from frontmatter
const posts = await getCollection('posts');

for (const post of posts) {
  if (post.data.redirect_from) {
    for (const oldUrl of post.data.redirect_from) {
...
<head>
  <meta charset="utf-8">
  <meta http-equiv="refresh" content="0; url=/[year]/[month]/[day]/${post.slug}/">
  <link rel="canonical" href="https://ben.balter.com/[year]/[month]/[day]/${post.slug}/">
</head>
<body>
  <p>Redirecting to <a href="/[year]/[month]/[day]/${post.slug}/">/[year]/[month]/[day]/${post.slug}/</a></p>
</body>
</html>
      `.trim();
      
      // Write to dist directory
      const redirectPath = path.join('dist', oldUrl, 'index.html');
...
// astro.config.mjs
export default defineConfig({
  // Run redirect generation after build
  integrations: [
    {
      name: 'generate-redirects',
      hooks: {
        'astro:build:done': async () => {
          await import('./scripts/generate-redirects.ts');
        },
      },
    },
  ],
...
3. **Post-build:** Generate redirects
   ```bash
...
```json
{
  "scripts": {
    "dev": "astro dev",
    "build": "npm run build:related-posts && astro build && npm run build:redirects",
    "build:related-posts": "tsx scripts/build-related-posts.ts",
    "build:redirects": "tsx scripts/generate-redirects.ts",
