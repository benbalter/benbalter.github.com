...
When you blindly optimize for one thing — appearances — behind the scenes there's a lot that goes unattended and it becomes increasingly complex to perform even the most simple of tasks. Extracting your content becomes tantamount to finding a needle in a purpose-built, legacy hatstack.
...
There's two things you'll notice there. First, the markup isn't semantic, meaning the presentation information is intermingled with the content, rending the authors intent indiscernible and using the content in any other context an increasingly difficult goal.
...
Next time you begin a new project for which the internet, not paper is not the primary output, think twice before firing up that desktop publishing platform. You'll gain more than mere semantics.
