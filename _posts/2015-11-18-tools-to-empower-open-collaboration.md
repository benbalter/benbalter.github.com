...
Developers may feel at home using GitHub, but that doesn't mean lawyers and marketers do too. Platform-agnostic, programatic access means you can [stand up tools to optimize the experience for a diversity of skillsets](http://ben.balter.com/2015/01/11/hacking-github/). Not to mention, APIs can allow you to present information in different ways such as high-level dashboards and bandwith- and remote-friendly mobile apps, further opening up the process. My general rule of thumb is to ask, "if we wanted, could we expose this information via ChatOps?".
...
* **RESTful API** - A complete and documented API avoid vendor/system lock in, and supports accessing the information from multiple platforms and in multiple formats
* **Cross-platform link notifications** - When you link to one issue on GitHub, from another, the target issue automatically notes the link, and the originating issue's status. This helps make links bidirection and surfaces the organization's knowledge graph without requiring an additional step.
