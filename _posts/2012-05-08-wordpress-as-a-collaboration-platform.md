---
id: 2460
author: 1
date: 2012-05-08 19:00:19
date_gmt: 2012-05-08 23:00:19
title: WordPress as a Collaboration Platform
excerpt: "Today countless teams are using WordPress to drive collaboration and facilitate inter-team communication. Ben Balter will showcase some of the creative ways companies and organizations are using WordPress as the central hub of their day-to-day workflow: To organize and collaboratively edit documents and other non-web content, track and communicate their team's progress with one another, and extend WordPress to work with their existing tools and practices."
status: publish
comment_status: open
ping_status: open
password:
name: wordpress-as-a-collaboration-platform
to_ping:
pinged: |
  http://themergency.com/generators/wordpress-custom-taxonomy/
  http://ben.balter.com/2011/08/29/wp-document-revisions-document-management-version-control-wordpress/
modified: 2012-08-14 11:27:17
modified_gmt: 2012-08-14 15:27:17
content_filtered:
parent: 0
guid: http://ben.balter.com/?p=2460
menu_order: 0
type: post
mime_type:
comment_count: 33
ancestors: [ ]
filter: raw
category:
  - Business
  - Technology
post_tag:
  - collaboration
  - document management
  - documents
  - edit flow
  - enterprise
  - gsoc
  - IT
  - members
  - open source
  - plugin
  - presentation
  - wiki
  - wordpress
  - workflow
post_format: [ ]
---
Today countless teams are using WordPress to drive collaboration and facilitate inter-team communication. This presentation — given at the May WordPress DC Meetup — showcases some of the creative ways companies and organizations are using WordPress as the central hub of their day-to-day workflow: To organize and collaboratively edit documents and other non-web content, track and communicate their team’s progress with one another, and extend WordPress to work with their existing tools and practices

*Click on the slides below to begin, then press the left/right keyboard arrows to navigate*

<div style="width: 100%; max-width: 550px; margin-bottom: 15px;">
  <p>
  </p>
  
  <p>
    <img id="collab_img" style="width: 100%; max-width: 550px; cursor: pointer;" src="http://benbalter.github.com/wordpress-as-a-collaboration-platform/screenshot.png" alt="" />
  </p>
  
  <p>
  </p>
  
  <p>
  </p>
  
  <div style="float: right; font-weight: bold;">
    <a href="http://benbalter.github.com/wordpress-as-a-collaboration-platform/" target="_BLANK">View Full Screen</a>
  </div>
  
  <div class="clearfix">
  </div>
</div>

Recording of the presentation (starts at the 10:00 mark):

<div style="text-align: center;">
  http://www.ustream.tv/recorded/22454841
</div>

## Plugins Mentioned

*   [WP Document Revisions][1]
*   [Edit Flow][2]
*   [Members][3]
*   [Advanced Custom Fields][4]
*   [Display Custom Fields][5]
*   [Count Shortcode][6]
*   [Faceted Search Widget][7]
*   [Custom Taxonomy Generator][8]

 [1]: http://ben.balter.com/2011/08/29/wp-document-revisions-document-management-version-control-wordpress/
 [2]: http://editflow.org
 [3]: http://wordpress.org/extend/plugins/members/
 [4]: http://wordpress.org/extend/plugins/advanced-custom-fields/
 [5]: http://wordpress.org/extend/plugins/wp-display-custom-fields/
 [6]: http://wordpress.org/extend/plugins/count-shortcode/
 [7]: http://wordpress.org/extend/plugins/faceted-search-widget/
 [8]: http://themergency.com/generators/wordpress-custom-taxonomy/