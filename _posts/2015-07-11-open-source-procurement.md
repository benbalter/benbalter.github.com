...
Instead of “50 hours of ₱ developer’s time”, an RFQ which is almost guaranteed to get you the cheapest developers, you buy "pull requests which fix issues [#1](https://github.com/benbalter/benbalter.github.com/issues/1), [#2](https://github.com/benbalter/benbalter.github.com/issues/2), and [#3](https://github.com/benbalter/benbalter.github.com/issues/3)", an approach which is more likely to get you the best possible solution to your problem. The idea is to treat government contractors as contributors, ones that happen to be paid for their time (rather than being motivated by a shared need). They select an issue, submitted a pull request, and the functionality is reviewed by stakeholders before being merged. It’s the government’s same requirements, testing and acceptance process, we’d just be doing it in the open, instead of in secret.
...
*I’d be remissed if I didn’t give a shoutout to David Hale ([**@lostonroute66**](https://github.com/lostonroute66)) whose trying to head down this path at NIH and whose conversation inspired this post.*
