...
description: "My book has a linter, ~5,500 automated tests, and a Pandoc pipeline that rebuilds five formats — EPUB, Kindle, paperback, and web — on every git push. For a book one person wrote. Here's how I built and published it the way I ship software."
...
My book has a linter that yells at me for hyphenating "open source."[^opensource] It runs ~5,500 automated checks, rebuilds five formats on every `git push`, and fails the build if I so much as imply I still work at a job I left. For a book. That one person wrote.
...
Not to mention, I had real-time feedback on my writing from the various prose linters, just as I would have real-time feedback on my code from ESLint or Prettier.
...
The linters caught mistakes, but they couldn't tell me whether the book repeated itself or whether a chapter was any good. For that I built a second layer of tooling: audits that judge the writing, not just check it. One thing they never did, though, was write it. Every word is mine; these tools are readers of last resort — catching what I'd stopped being able to see — not writers of first resort.
