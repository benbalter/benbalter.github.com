...
description: "The ebook conforms to WCAG 2.1 Level AA because the format wouldn't let me fake a heading and a browser audits every build, not because anyone scheduled a week for it. What Markdown can't check—color contrast, generated markup—axe-core checks under Playwright."
...
What none of it can see is anything that isn't text to the browser. A QR code is a grid of `<rect>` elements: no text node, no accessible name, nothing for a contrast rule to have an opinion about. Catching that one was my own rule's job, and my own rule was too specific.[^qr]
