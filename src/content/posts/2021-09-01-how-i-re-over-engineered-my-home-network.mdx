...
description: How I used Docker Compose, Ansible, and Caddy to re-over-engineer my network setup for easier maintenance, treating my Raspberry Pi like cattle, not a pet.
tldr: "How I used Docker Compose, Ansible, and Caddy to re-over-engineer my home network setup for easier maintenance, treating my Raspberry Pi like cattle, not a pet."
---
...
What I wrote then remains true, but after having relied on, optimized, and upgraded what I described in my previous post for about eighteen months now, I've decided to build on what's there by ~~revisiting~~ re-over-engineering how I setup, maintain, and manage the software and services that power and protect the network with a number of specific goals in mind:
...
:::info[Edit — 2021–11–04]
Since originally publishing this post, I've swapped out Pi-Hole + Cloudflared in favor of [AdGuard Home](https://github.com/AdguardTeam/AdGuardHome), and haven't looked back. While the functionality is largely comparable at this point, and ultimately you could be happy with either, I ended up preferring AdGuard Home for a number of reasons:
...
<details class="mb-3">
<summary>Example <code>docker-compose.yml</code> file</summary>
...
In short, less muxing with the underlying system and better-defined interfaces instills more confidence that services are well-understood and that things are working as intended.

:::info[Update — August 2026]
In the years since, that single `docker-compose.yml` has quietly grown well past DNS. The Raspberry Pi now doubles as a home-automation hub: [Home Assistant](https://www.home-assistant.io/), an MQTT broker ([Mosquitto](https://mosquitto.org/)), a Zigbee bridge ([Zigbee2MQTT](https://www.zigbee2mqtt.io/)), and an uptime dashboard all live as additional services in the same file — each still isolated, still pinned to an image, still torn down and rebuilt without touching the host. I also added [Watchtower](https://containrrr.dev/watchtower/), which pulls fresh images and recreates containers on a schedule, finally paying off the original "systems would update themselves" goal without me in the loop.
:::
...
:::info[Update — August 2026]
Ansible handles the Pi, but the network hardware it talks to was still configured by hand, click by click, in the router's web UI. Since writing this, I've pushed "infrastructure as code" one layer down: the UniFi Dream Machine's own configuration — the networks, the WLANs, and the zone-based firewall — is now managed with [OpenTofu](https://opentofu.org) (the open-source Terraform fork), with its state and plans **encrypted at rest** so nothing sensitive ever lands in the repo in the clear.

The most satisfying payoff is how it hardened the DNS story. Originally I simply pointed clients at the Pi and trusted them to comply; a determined device (or a chatty smart TV) could just as easily reach out to `8.8.8.8` on its own. Now a set of firewall policies actively _block_ the usual ways around it — plain DNS, DNS-over-TLS, DNS-over-QUIC, and a curated list of known public resolver IPs — so a device can't just point itself elsewhere and wander off, with a narrow exception for the resolver itself to reach its upstream. (Encrypted DoH tunneled over ordinary port-443 HTTPS remains the hard case — it's indistinguishable from any other web traffic — but it's now the exception rather than the wide-open default.) Codified, version-controlled, and enforced at the router instead of politely requested.
:::
...
<details class="mb-3">
<summary>Example Ansible <code>playbook.yml</code> file for running Docker Compose services</summary>
...
Beyond the above, here are a few nice automations to simplify provisioning and maintenance:

<details class="mb-3">
<summary>Additional convenience task to add to your Ansible <code>playbook.yml</code> file</summary>
...
<details class="mb-3">
<summary>Security best practices I included in my Ansible <code>playbook.yml</code> file</summary>
...
<details class="mb-3">
<summary>Example <code>docker-compose.yml</code> additions to support Caddy/HTTPS</summary>
...
<details class="mb-3">
<summary>Example <code>caddy.Dockerfile</code> to build Caddy with custom challenge support</summary>
...
<details class="mb-3">
<summary>Example <code>caddyfile</code> to proxy HTTPS to the PiHole or AdGuard Home web interface</summary>
...
<details class="mb-3">
<summary>Example Ansible <code>playbook.yml</code> task to expose Cloudflare Token to Caddy</summary>
...
:::info[Update — August 2026]
Nearly five years on, this is all still humming along — which, given the original "set it and forget it" goal, might be the highest praise I can give it. The stack has outgrown its name (it's less "pi-hole" and more "the box that runs the house" these days), but the principles held: every change is still a reviewable diff, upgrades are still boring, and I still haven't lost a weekend to a surprise dependency conflict. The repo linked below now lives at [`benbalter/pi-hole`](https://github.com/benbalter/pi-hole) and has grown to include the OpenTofu network configuration alongside the Docker and Ansible bits.
:::

Eighteen months since [I originally overengineered my home network](https://ben.balter.com/2020/12/04/over-engineered-home-network-for-privacy-and-security/), ads remain rare, false positives are still low, and I've learned a lot about many of the behind-the-scenes technologies we've come to take for granted every day. If you're looking to implement a similar set up (or do and find ways to improve it), you can find all the configuration files referenced above (and more!) over at [benbalter/pi-hole-cloudflared-docker-compose-ansible-caddy](https://github.com/benbalter/pi-hole-cloudflared-docker-compose-ansible-caddy) on GitHub.
